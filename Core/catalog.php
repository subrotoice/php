...
$total_pages = 1;
$offset = 0;
if ($total_items > 0) {  // if total_items 0 hoy then skep this calculation
  $total_pages = ceil($total_items / $items_per_page);

  //limit results in redirect
  $limit_results = "";
  if(!empty($search)){
    $limit_results = "s=" . $search . "&";
  } else if (!empty($section)) {
    $limit_results = "cat=" . $section . "&";
  }
  // redirect to the last page when page number grater then total pages
   if ($current_page > $total_pages) {
     header("location:catalog.php?"
     . $limit_results
     . "pg=" .$total_pages);
   }
  // redirect to the first page when page nubmer less then one
   if ($current_page < 1) {
...
<div class="section catalog page">
    ...
    <div class="wrapper">
        ...
        <h1><?php
        if (!empty($search)) {
          echo "Search restult for \"". htmlspecialchars($search) . "\"";
        } else {
        if ($section != null) {
        ...
        <?php
        if ($total_items < 1) {
          echo "<p>No items were found matching that search item</p>";
          echo "<p>Search Again <a href='catalog.php'>Browse Full Category</a></p>";
        } else {
        echo $pagination; ?>
        <ul class="items">
            <?php
